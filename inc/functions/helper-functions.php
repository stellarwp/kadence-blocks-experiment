...
function kbs_wc_clean( $var ) {
	if ( is_array( $var ) ) {
		return array_map( 'kbs_wc_clean', $var );
	} else {
...
function kbs_get_current_license_email() {
	if ( ! empty( get_license_key( 'kadence-blocks-pro' ) ) ) {
		...
		$license_data = kbs_get_deprecated_pro_license_data();
 ...
 */
function kbs_get_current_license_data(): array {
	static $cache;
	...
	$license_data = [
		'key'     => kbs_get_current_license_key(),
		'email'   => kbs_get_current_license_email(),
		'product' => kbs_get_current_product_slug(),
	];
...
function kbs_get_deprecated_pro_license_data() {
	...
	} elseif ( is_multisite() && kbs_is_network_authorize_enabled() ) {
