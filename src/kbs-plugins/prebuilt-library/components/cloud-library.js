...
const CloudLibrary = ({ searchTerm, onSelect }) => {
	...
	const { cloudItems, isLoading, error, isConnected, connectCloud } = useCloudData(cloudKey);
	...
	if (!isConnected) {
		return (
			<div className="kadence-cloud-library-connect">
				...
				<p>
					{__('Enter your Kadence Cloud key to access your saved templates and patterns.', 'kadence-blocks')}
				</p>
				...
				<Button variant="primary" onClick={handleConnect} disabled={!cloudKey}>
	...
	return (
		<div className="kadence-cloud-library">
			...
			<Grid columns={3} gap={4} className="kadence-cloud-library-grid">
				{cloudItems
					.filter((item) => !searchTerm || item.title.toLowerCase().includes(searchTerm.toLowerCase()))
					.map((item) => (
						<Card key={item.id} className="kadence-cloud-library-item" isElevated>
							<CardMedia>
