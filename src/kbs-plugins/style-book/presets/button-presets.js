import { __ } from '@wordpress/i18n';
import { useSelect, useDispatch } from '@wordpress/data';
import { useEffect } from '@wordpress/element';
import { Button, Dropdown } from '@wordpress/components';
import { plus } from '@wordpress/icons';

import { BLOCK_COMPONENTS, ButtonPresetRender, TextControl } from '@kadence/kbsComponents';
import { getPresetOptions } from '@kadence/kbsHelpers';

import './editor.scss';
/**
 * Build the component preset
 */
export default function ButtonPresets(props) {
	const {
		setStyleBookAttributes,
		setSelectedComponent,
		globalStyleId,
		currentPreset,
		previewDevice,
		startNewPreset,
		newPresetName,
		setNewPresetName,
	} = props;

	const presets = getPresetOptions('buttonVariant');
	const meta = {
		name: 'buttonVariant',
		debug: true,
		attributes: {
			typography: {
				component: 'typography',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				initial: {},
				type: 'object',
			},
			color: {
				component: 'color',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			backgroundColor: {
				component: 'color',
				renderCSS: true,
				type: 'object',
				varPrefix: '--kbs-button-background-',
				classPrefix: 'kbs-button-background-color',
			},
			border: {
				component: 'border',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			padding: {
				component: 'padding',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			margin: {
				component: 'margin',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			transform: {
				component: 'transform',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			transition: {
				component: 'transition',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			boxShadow: {
				component: 'box-shadow',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				type: 'object',
			},
			boxShadowHover: {
				component: 'box-shadow',
				renderCSS: true,
				varPrefix: '--kbs-button-',
				varSuffix: '-hover',
				classPrefix: 'kbs-button-shadow-hover-',
				type: 'object',
			},
		},
	};
	return (
		<div className="kbs-button-presets kbs-preset-control kbs-control">
			<div className="kbs-storybook-section-header kbs-button-presets-header">
				<h3 className="kbs-storybook-header-title kbs-button-presets-header-title">
					{__('Button Presets', 'kadence-blocks')}
				</h3>
				{globalStyleId === 'kbs-base' && (
					<Dropdown
						popoverProps={{
							placement: 'left-start',
							//offset: 36,
							shift: true,
						}}
						className={'kbs-popover-add-global-style'}
						contentClassName={'kbs-popover-add-global-style-content'}
						renderToggle={({ isOpen, onToggle }) => (
							<Button
								icon={plus}
								className="kbs-advanced-controls-button"
								onClick={onToggle}
								isPressed={isOpen}
								variant="secondary"
								aria-expanded={isOpen}
								iconSize={18}
								text={__('Add Button Preset', 'kadence-blocks')}
							/>
						)}
						renderContent={({ isOpen, onToggle }) => (
							<div className="kbs-popover-add-global-style-content">
								<h2 className="kbs-popover-add-global-style-content-title">
									{__('Add Button Preset', 'kadence-blocks')}
								</h2>
								<div className="kbs-popover-add-global-style-content-items kbs-control">
									<TextControl
										label={__('Preset Name', 'kadence-blocks')}
										value={newPresetName}
										onChange={(value) => setNewPresetName(value)}
									/>
									<div className="kbs-popover-add-global-style-content-items-buttons">
										<Button
											variant="primary"
											disabled={!newPresetName}
											onClick={() => {
												startNewPreset();
												onToggle();
											}}
										>
											{__('Add Preset', 'kadence-blocks')}
										</Button>
										<Button __next40pxDefaultSize onClick={onToggle}>
											{__('Cancel', 'kadence-blocks')}
										</Button>
									</div>
								</div>
							</div>
						)}
					/>
				)}
			</div>
			<div className="kbs-control-inner kbs-button-presets-grid">
				{presets.map((option) => (
					<Button
						key={option.value}
						label={option.label}
						isPressed={option.value === currentPreset}
						className={`kbs-radio-preset-control-button`}
						onClick={() => {
							setSelectedComponent('buttonVariant');
							setStyleBookAttributes({
								components: { buttonVariant: { selectedPreset: option.value } },
							});
						}}
					>
						<ButtonPresetRender
							isCurrentPreset={option.value === currentPreset}
							preset={option}
							attributeName={'buttonVariant'}
							meta={meta}
							previewDevice={previewDevice}
							globalStylesIds={[globalStyleId]}
							globalStyleId={globalStyleId}
							uniqueID={globalStyleId + '-button-preset-' + option.value}
							className={`kbs-radio-preset-control-style`}
						/>
					</Button>
				))}
			</div>
		</div>
	);
}
