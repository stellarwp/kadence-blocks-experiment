...
export default function parseBorderStyle(borderStyle) {
	...
	if (parts.length === 3) {
		currentWidthValue = parts[0].includes(BORDER_STYLES_DEFAULTS.width) ? '' : parts[0];
		currentStyleValue = parts[1].includes(BORDER_STYLES_DEFAULTS.style) ? '' : parts[1];
		currentColorValue = parts[2].includes(BORDER_STYLES_DEFAULTS.color) ? '' : parts[2];
	}
