...
export class BackgroundGenerator extends BaseComponentGenerator {
	...
	processStandardMask(layer, tempSelector, maskColor) {
		...
		this.add({
			'mask-image': `url("data:image/svg+xml, ${encodeURIComponent(
				'<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="' +
					ratio +
					'" viewBox="0 0 1920 1200" fill="black"><path d="' +
					mask.path +
					'" /></svg>'
			)}")`,
			'mask-repeat': 'no-repeat',
		...
		if (maskAlignX === 'min') {
			positionX = 'left';
		} else if (maskAlignX === 'max') {
			positionX = 'right';
		}
		if (maskAlignY === 'min') {
			positionY = 'top';
		} else if (maskAlignY === 'max') {
			positionY = 'bottom';
		}
	...
	processBackdropHover(layer) {
		...
		const backdropFilterHover =
			getLayerDeviceValue('backdropFilterHover', layer, this.props.previewDevice) || backdropFilter;
		...
		if (backdropFilterHover) {
			if (backdropFilterHover === 'none') {
				...
				const backdropSize = getLayerDeviceValue('backdropSize', layer, this.props.previewDevice) || '1';
