...
function TransformScale(props) {
	...
	const handleMouseMove = useCallback(
		(e) => {
			if (!isDragging || !dragStartRef.current || !dragCorner) {
				return;
			}

			const { centerX, centerY, baseDistX, baseDistY, initialScaleX, initialScaleY, initialRatio } =
				dragStartRef.current;
	...
	return (
		<div className="kadence-transform-scale-content">
			<div className="kadence-transform-title-wrapper">
				<span className="kadence-transform-scale-title">{__('Transform Scale', 'kadence-blocks')}</span>
				<Button
