...
const getColorMixValues = (value) => {
	...
	if (value.startsWith('color-mix')) {
		...
		return ['', 'var(--kbs-mix-black, #000000)', '50%'];
	}
...
const ColorMix = ({ onChange, value, globalClasses, isHover, inherited, globalStylesCss }) => {
	...
	const isMix =
		type &&
		type !== 'var(--kbs-mix-transparent, transparent)' &&
		type !== 'var(--kbs-mix-white, #ffffff)' &&
		type !== 'var(--kbs-mix-black, #000000)';
	const typeValue = useMemo(() => {
		if (!type) {
			return '';
		}
		if (type === 'var(--kbs-mix-black, #000000)' || type === 'var(--kbs-mix-white, #ffffff)') {
			return 'shade';
		}
		if (type === 'var(--kbs-mix-transparent, transparent)') {
			return 'transparent';
		}
		return 'mix';
	}, [type]);
	const inheritedTypeValue = useMemo(() => {
		...
		if (inheritedType === 'var(--kbs-mix-black, #000000)' || inheritedType === 'var(--kbs-mix-white, #ffffff)') {
			return 'shade';
		}
		if (inheritedType === 'var(--kbs-mix-transparent, transparent)') {
			return 'transparent';
		}
		return 'mix';
	}, [inheritedType]);
